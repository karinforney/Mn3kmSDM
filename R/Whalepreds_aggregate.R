# RAIMBOW pre-processing function: whalepreds_aggregate
...
###############################################################################
whalepreds_aggregate_dates <- function(x.names, x.col.idx, 
                                       aggr.level = NULL, 
                                       range.dates = NULL) {
  ...
  # x.col.idx: indices of characters in column names that specify the date;
  ...
  #----------------------------------------------------------------------------
  ### Prep: generate dates to define specified intervals
  if (is.null(range.dates)) {
    # Date range NOT is manually specified
    stopifnot(
      aggr.level %in% c("monthly", "biweekly") | grepl("day", aggr.level)
    )
    ...
    } else if (aggr.level == "biweekly") {
      range.dates <- seq(from = min(cols.dates), to = max(cols.dates), by = "2 weeks")
      range.dates <- c(range.dates, max(range.dates) + days(14))
      
    } else if (grepl("day", aggr.level)) {
      day.int <- as.numeric(strsplit(aggr.level, "day")[[1]][1])
      if (!inherits(day.int, "numeric")) {
        stop("If aggregating by some number of days, please ensure aggr.level ", 
             "is in the form '#day', e.g. \"10 days\" or \"1 day\"")
      }
      range.dates <- seq(from = min(cols.dates), to = max(cols.dates), by = paste(day.int, "day"))
      range.dates <- c(range.dates, max(range.dates) + days(day.int))
      
    } else {
      stop("Error in aggr.level argument; see functions docs")
    }
    ...
    x, x.cols, x.col.idx, aggr.level = NULL, range.dates = NULL, se.calc = FALSE) 
{
  ...
  if (se.calc) {
    data.summ.se <- data.frame(apply(x.key.summ, 1, function(i, j) {
      # apply call take < 1s per month aggr, pmap call takes 1.5s per aggr
      x1 <- apply(j[,  i[["x_data_cols_list"]]], 1, raimbow_se)
    }, j = x.data)) %>% 
